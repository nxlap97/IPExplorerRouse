...
$(document).ready(function () {
    ...
    setTimeout(function () {
        $("#people-search-result .slick-arrow").addClass("hide")
    }, 400)
    ...
    $(document).ready(function () {
        ...
        var id = url.substring(url.lastIndexOf('?') + 1);//check id after #
        var ulServicesMenu = $("ul[aria-labelledby='services-menu'] li");
        var strServiceName = "Service=";
        if (url.indexOf('#') > 0) { id = url.substring(url.lastIndexOf('#') + 1); }
        switch (id) {
            case strServiceName + 'Corporate+&+Commercial': ulServicesMenu.eq(0).click(); break; //click ul li selecting 1 when id == Service=Commercial
            case strServiceName+'Copyright': ulServicesMenu.eq(1).click(); break;
            case strServiceName+'Designs': ulServicesMenu.eq(2).click(); break;
            case strServiceName+'Disputes': ulServicesMenu.eq(3).click(); break;
            case strServiceName+'Enforcement': ulServicesMenu.eq(4).click(); break;
            case strServiceName+'Geographical+Indications': ulServicesMenu.eq(5).click(); break;
            case strServiceName+'IP+Consultancy': ulServicesMenu.eq(6).click(); break;
            case strServiceName+'Management+Consultancy': ulServicesMenu.eq(7).click(); break;
            case strServiceName +'E-Commerce,+Marketing+&+Digital': ulServicesMenu.eq(8).click(); break;
            case strServiceName+'Technology+&+Patents': ulServicesMenu.eq(9).click(); break;
            case strServiceName+'Sport+IP': ulServicesMenu.eq(10).click(); break;
            case strServiceName+'Trade+Marks': ulServicesMenu.eq(11).click(); break;
          
            case 'what': $('html, body').animate({                                    //scroll page to id == whatppening
...
$("#touch-menu-trigger").click(function () {
    ...
    if (windowWidth <= 1200) {
        ...
        if (status !== "true") {
           ...
           // $("#burger-menu li a[url-string = " + locationCurrent + "],a[is-redirect=False]").click();
...
//People mobile slick
$('.people-search-result-slick-mobile').on('swipe', function (event, slick, direction) {
...
$('#people-search-result').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
    ...
    if ((nextSlide + 1) == slick.slideCount) {
        ...
        if (currentPage < totalPage) {
            $("#people-search-result .slick-next").removeClass("hide");
        ...
        if (!$("#people-search-result .slick-next").hasClass("hide")) {
            $("#people-search-result .slick-next").addClass("hide");
        }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (Total == 0) {
                ...
                if (Total > Page) {
                    $(".load-more-list").removeClass("hide");
                }
...
function renderItemPeople(data) {
    ...
    let content = '<div class="col-md-6 col-lg-3"> <div class="people-item" style="background-image:url()">' +
        ...
        '     <ul class="linkedin-profile-url">' + htmlList +
        '</ul>' +
        ' </div>' +
        '</div>' +
        ' </div>' +
